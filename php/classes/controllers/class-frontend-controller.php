...
use PdSPodcast\ShortCodes\Player;
use PdSPodcast\ShortCodes\Podcast;
use PdSPodcast\ShortCodes\Podcast_Episode;
use PdSPodcast\ShortCodes\Podcast_Playlist;

use PdSPodcast\Widgets\Playlist;
use PdSPodcast\Widgets\Series;
use PdSPodcast\Widgets\Recent_Episodes;
use PdSPodcast\Widgets\Single_Episode;
...
class Frontend_Controller extends Controller
{
	...
	public function register_html5_player_assets()
	{
		 ...
		 */
		if (has_block('pds-podcast/castos-player') || has_block('pds-podcast/podcast-list')) {
	...
	public function validate_media_player($episode_id, $show_player = true)
	{
		 ...
		 */
		if (has_block('pds-podcast/castos-player')) {
			$show_player = false;
		}
	...
	protected function get_meta_display( $podcast_display, $subscribe_display ) {
		...
		if ( ! empty( $podcast_display ) || ! empty( $subscribe_display ) ) {
			...
			if ( ! empty( $subscribe_display ) ) {
				$subscribe_display = '<p>' . __( 'Subscribe:', 'pds-podcast' ) . ' ' . $subscribe_display . '</p>';
			...
			switch ( $key ) {
				...
				case 'link':
					if ( 'on' === get_option( 'ss_podcasting_download_file_enabled', 'on' ) ) {
						$podcast_display .= $sep . '<a href="' . esc_url( $data ) . '" title="' . get_the_title() . ' " class="podcast-meta-download">' . __( 'Download file', 'pds-podcast' ) . '</a>';
					}
					break;
				...
				case 'new_window':
					if ( isset( $meta['link'] ) && 'on' === get_option( 'ss_podcasting_play_in_new_window_enabled', 'on' ) ) {
						...
						$podcast_display .= $sep . '<a href="' . esc_url( $play_link ) . '" target="_blank" title="' . get_the_title() . ' " class="podcast-meta-new-window">' . __( 'Play in new window', 'pds-podcast' ) . '</a>';
					}
					break;

				case 'duration':
					if ( 'on' === get_option( 'ss_podcasting_duration_enabled', 'on' ) ) {
						$podcast_display .= $sep . '<span class="podcast-meta-duration">' . __( 'Duration', 'pds-podcast' ) . ': ' . $data . '</span>';
					}
					break;

				case 'date_recorded':
					if ( 'on' === get_option( 'ss_podcasting_date_recorded_enabled', 'on' ) ) {
						$podcast_display .= $sep . '<span class="podcast-meta-date">' . __('Recorded on', 'pds-podcast') . ' ' . date_i18n(get_option('date_format'), strtotime($data)) . '</span>';
					}
					break;
	...
	/**
	 * Public action which is triggered from the PdS Hosting queue
	 * Imports episodes to PdS Hosting
	 */
	...
	public function generator_tag($gen, $type)
	{
		...
		if (apply_filters('ssp_show_generator_tag', true, $type)) {
			...
			$generator = 'PdS Podcast ' . esc_attr($this->version);
	...
	public function rss_meta_tag()
	{
		...
		if (apply_filters('ssp_show_global_feed_tag', true)) {
			$html = '<link rel="alternate" type="application/rss+xml" title="' . __('Podcast RSS feed', 'pds-podcast') . '" href="' . esc_url($feed_url) . '" />';
		}
		...
		if (isset($current_obj->taxonomy) && 'series' == $current_obj->taxonomy && isset($current_obj->slug) && $current_obj->slug) {
			...
			if (apply_filters('ssp_show_series_feed_tag', true, $current_obj->slug)) {
				...
				$html .= "\n" . '<link rel="alternate" type="application/rss+xml" title="' . sprintf(__('%s RSS feed', 'pds-podcast'), $current_obj->name) . '" href="' . esc_url($series_feed_url) . '" />';
	...
	{
		load_plugin_textdomain('pds-podcast', false, basename(dirname($this->file)) . '/languages/');
	}
	 ...
	 * This is used in the PdSPodcast\Widgets\Single_Episode widget
	 * as well as the PdSPodcast\ShortCodes\Podcast_Episode shortcode
	 *
	 * @param integer $episode_id ID of episode post
